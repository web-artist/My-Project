...
/* load more post */function more_post_ajax(){$cat= 79;	$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;	    $offset = $_POST["offset"];    $ppp = $_POST["ppp"];    header("Content-Type: text/html");		$args = array(    'post_type'             => 'product',    'post_status'           => 'publish',    'ignore_sticky_posts'   => 1,    'posts_per_page'        => $ppp,	'offset'                => $offset,	'paged'                 => $paged,		'tax_query'             => array(        array(            'taxonomy'      => 'product_cat',            'field' => 'term_id',             'terms'         => $cat,            'operator'      => 'IN'         )    )	);	 $loop = new WP_Query( $args ); 		if($loop->have_posts()){	while ( $loop->have_posts() ) : $loop->the_post(); global $product; ?>      <li class="cus_col-sm-3 cat-item1 myproducts">                        <a href="<?php echo get_permalink( $loop->post->ID ) ?>" title="<?php echo esc_attr($loop->post->post_title ? $loop->post->post_title : $loop->post->ID); ?>">                        <?php woocommerce_show_product_sale_flash( $post, $product ); ?>                        <?php if (has_post_thumbnail( $loop->post->ID )) echo get_the_post_thumbnail($loop->post->ID, 'shop_catalog'); else echo '<img src="'.woocommerce_placeholder_img_src().'" alt="Placeholder" width="300px" height="300px" />'; ?>                        <h3><?php the_title(); ?></h3>                        <span class="price"><?php echo $product->get_price_html(); ?></span>                                        </a>                    <?php woocommerce_template_loop_add_to_cart( $loop->post, $product ); ?>	</li>    <?php endwhile; ?>    <?php wp_reset_query();}else{	echo '<div class="col-md-12 no-more-posts"> No More Products Available. </div>';}		}	add_action('wp_ajax_nopriv_more_post_ajax', 'more_post_ajax'); add_action('wp_ajax_more_post_ajax', 'more_post_ajax');/* ****END*****  */
...
/* load more post ajax*/
function lazy_load_product_tag(){
	$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;	
    $offset = $_POST["offset"];  
	$ppp = $_POST["ppp"]; 
	$cat_id= $_POST["cat_id"];	
...
add_action('wp_ajax_nopriv_lazy_load_product_tag', 'lazy_load_product_tag');
add_action('wp_ajax_lazy_load_product_tag', 'lazy_load_product_tag');
	/* ****END*****  */
	
	/************************Lazy load color filter****************************/
	
	/* load more post ajax*/
function lazy_load_color_filter(){
	$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;	
    $offset = $_POST["offset"];  
	$ppp = $_POST["ppp"]; 
	$cat_id= $_POST["cat_id"];
	$color_name = $_POST["color_name"];			
	$color_min_price = $_POST["color_min_price"];			
	$color_max_price = $_POST["color_max_price"];			
	header("Content-Type: text/html");	
	$args = array( 
...
add_action('wp_ajax_nopriv_lazy_load_color_filter', 'lazy_load_color_filter');
add_action('wp_ajax_lazy_load_color_filter', 'lazy_load_color_filter');
	/* ****END*****  */
	
	/************************Lazy load price filter****************************/
...
add_action('wp_ajax_nopriv_lazy_load_price_filter', 'lazy_load_price_filter');
add_action('wp_ajax_lazy_load_price_filter', 'lazy_load_price_filter');
	
				/*********END*******************/
				

	/************************Lazy load diamond filter****************************/
		
function lazy_load_diamond_filter(){
	$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;	
    $offset = $_POST["offset"];  
	$ppp = $_POST["ppp"]; 
	$cat_id= $_POST["cat_id"];
	$diamond_opt = $_POST["diamond_opt"];						
	header("Content-Type: text/html");	
	$args = array( 
	'post_type'             => array('product'),
    'post_status'           => 'publish',
...
add_action('wp_ajax_nopriv_lazy_load_diamond_filter', 'lazy_load_diamond_filter');
add_action('wp_ajax_lazy_load_diamond_filter', 'lazy_load_diamond_filter');
				/* ****END*****  */
	
				/************************Lazy load color & Price filter****************************/
	
function lazy_load_color_price_filter(){
	$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;	
    $offset = $_POST["offset"];  
	$ppp = $_POST["ppp"]; 
	$cat_id= $_POST["cat_id"];
	$prc_color_name = $_POST["prc_color_name"];			
	$color_min_price = $_POST["color_min_price"];			
	$color_max_price = $_POST["color_max_price"];			
	header("Content-Type: text/html");	
	$args = array( 
...
add_action('wp_ajax_nopriv_lazy_load_color_price_filter', 'lazy_load_color_price_filter');
add_action('wp_ajax_lazy_load_color_price_filter', 'lazy_load_color_price_filter');
	
						/* ****END*****  */
	
/************************Lazy load color & Diamonds filter****************************/
	
function lazy_load_color_diamond_filter(){
	$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;	
    $offset = $_POST["offset"];  
	$ppp = $_POST["ppp"]; 
	$cat_id= $_POST["cat_id"];
	$prc_color_name = $_POST["prc_color_name"];						
	$filter_atr_val = $_POST["filter_atr_val"];			
	header("Content-Type: text/html");	
	$args = array( 
	'post_type'             => array('product'),
    'post_status'           => 'publish',
...
add_action('wp_ajax_nopriv_lazy_load_color_diamond_filter', 'lazy_load_color_diamond_filter');
add_action('wp_ajax_lazy_load_color_diamond_filter', 'lazy_load_color_diamond_filter');
	
						/* ****END*****  */	

				/************************Lazy load Price & Diamond filter****************************/
	
function lazy_load_price_diamond_filter(){
	$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;	
    $offset = $_POST["offset"];  
	$ppp = $_POST["ppp"]; 
	$cat_id= $_POST["cat_id"];
	$filter_price_diamond_val = $_POST["filter_price_diamond_val"];			
	$diamond_min_price = $_POST["diamond_min_price"];			
	$diamond_max_price = $_POST["diamond_max_price"];			
	header("Content-Type: text/html");	
	$args = array( 
	'post_type'             => array('product'),
    'post_status'           => 'publish',
...
add_action('wp_ajax_nopriv_lazy_load_price_diamond_filter', 'lazy_load_price_diamond_filter');
add_action('wp_ajax_lazy_load_price_diamond_filter', 'lazy_load_price_diamond_filter');
...
$my_need = home_url( $wp->request )
if($my_need == "https://alor.com/shop"){
function show_woo_prodt() {
...
add_action( 'init', 'show_woo_prodt' );
} */ 
/* 
